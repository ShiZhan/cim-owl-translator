...
CIM     = Namespace("http://www.storagekb.org/ontologies/2011/cim_smis.owl")

# Create an identifier to use as the subject for ontology.
#ontology = BNode()

# declare metadata
store.add((CIM, RDF.type, OWL["Ontology"]))
store.add((CIM, RDFS["comment"], Literal("Computer system management ontology"
    " translated from DMTF CIM Schema, all in one version.", lang="EN")))
store.add((CIM, DC["date"], Literal("2011-3-28")))
store.add((CIM, DC["creator"], Literal("Shi.Zhan")))
store.add((CIM, DC["created"], Literal(
    time.strftime(u'%Y-%m-%d %H:%M:%S'.encode('utf-8'),
    time.localtime(time.time())).decode('utf-8'))))
store.add((CIM, TERMS["license"], Literal("Copyright 2011 Shi.Zhan."
